...
```{r}
religion_income <- readr::read_csv("data/relig_income.csv")
religion_income
```

2. Why are these data untidy?  

this data is untidy because the column names are values 
...
```{r}
plants <- readr::read_csv ("data/plant_data.csv")
plants
```

2. Why are these data not tidy?  

the days are shown as variables

3. Use `pivot_longer()` to make the data tidy. Focus the data only on genotype, day, and measurement.  
```{r}
plants2 <- plants %>%
  pivot_longer(day1:day30, names_to = "day", values_to = "number", names_prefix = "day")
plants2
```
