...
homerange %>% 
  ...
  geom_density(fill="salmon", alpha  =0.4, color = "black")+
  labs(title = "log10.hra")
```
...
homerange %>% 
  ggplot(aes(x = log10.hra)) +
  geom_density(fill="salmon", alpha  =0.4, color = "black")+ geom_histogram(aes(y = ..density..), fill = "magenta", alpha = 0.4, color = "black")
  labs(title = "log10.hra") 
```
...
homerange1 <- homerange %>% 
             mutate(range_category = case_when(log10.hra <= 0.05 ~ "very small",
                                              log10.hra > 0.05 & log10.hra <= 0.1 ~ "small",
                                              log10.hra > 0.1 & log10.hra <= 0.15 ~ "medium",
                                              log10.hra > 0.2 ~ "large"))
homerange1
```
...
```{r}
homerange2 %>%
  ggplot(aes(x=range_category, fill= range))+
  geom_bar(position = "dodge", color="black", alpha=0.6) +
  labs(title = "range category"),
        x = "Mass Category",
       fill = "Trophic Guild
```
