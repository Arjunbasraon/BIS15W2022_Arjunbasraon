...
```{r}
p+theme_linedraw()+
  theme_economist(= element_text(angle = 60, hjust=1))+
...
```
```{r}
display.brewer.simpsons(8,"PRGn")
```
...
```{r}
p2+ facet_wrap(mean.mass.g~taxon)
```
...
build a faceting plot that shows a distribution of life expectancy.
