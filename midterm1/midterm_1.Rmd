...
```
```{r}
mean(near$RA_Elephant)
```
```{r}
mean(near$RA_Monkeys)
```
```{r}
mean(near$RA_Rodent)
```
```{r}
mean(near$RA_Ungulate)
```
...
RA_Apes <- c(0.12, 4.91)
RA_Apes
RA_Birds <- c(76.64 ,31.56 )
RA_Birds
RA_Elephant <- c(0.145 ,0 )
RA_Elephant
RA_Monkeys <- c(17.335 ,54.12 )
RA_Monkeys
RA_Rodent <- c(3.895 ,1.29 )
RA_Rodent
RA_Ungulate <- c(1.87 ,8.12)
RA_Ungulate
```
```{r}
comparision <- data.frame(distance, RA_Apes, RA_Birds, RA_Elephant, RA_Monkeys, RA_Rodent, RA_Ungulate)
comparision
```
