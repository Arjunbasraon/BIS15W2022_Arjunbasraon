...
ui <- fluidPage(radioButtons("x", "Select Fill Variable", choices = c("ethnicity", "academic_yr", "campus", "filtered_count_fr"), 
...
server <- function(input, output, session) {
  output$plot <- renderPlot({
    ggplot(data=cleanUC, aes_string(x="perc_fr", fill=input$x))+
...
```{r, eval=F, echo=T}
ui <- fluidPage(
    selectInput("x", "Select X Variable", choices = c("academic_yr", "ethnicity", "campus", "perc_fr"),
              selected = "campus"),
    selectInput("y", "Select Y Variable", choices = c("academic_yr", "ethnicity", "campus", "perc_fr"),
              selected = "campus"),
...
server <- function(input, output) {
  output$plot <- renderPlot({
    ggplot(data=cleanUC, aes_string(x = input$x, y = input$y, color = "ethnicity")) + 
    geom_point(color="black", fill="red", alpha=0.6) + 
